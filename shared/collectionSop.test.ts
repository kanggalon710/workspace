...
const LADDER: SopStageMeta[] = [
  ...
  { key: "delegasi_cs", ownerDivision: "cs", slaDays: 7, nextStageKey: "delegasi_marketing", role: "none" },
  { key: "delegasi_marketing", ownerDivision: "marketing", slaDays: 7, nextStageKey: null, role: "none" },
...
const terminal = new Set(["paid", "written_off"]);
...
test("tepat di ambang SLA (>=) → advance", () => {
  ...
  assert.equal(decideSopAdvance("delegasi_cs", 7.01, byKey, terminal).toStage, "delegasi_marketing");
});
...
test("stage akhir ladder (delegasi_marketing) tanpa next → no_next", () => {
  const d = decideSopAdvance("delegasi_marketing", 30, byKey, terminal);
  assert.equal(d.advance, false);
  assert.equal(d.reason, "no_next");
});
